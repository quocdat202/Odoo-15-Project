odoo.define('reports_designer.BasicModelParam', function (require) {
"use strict";
...
BasicModel.include({
	_fetchSpecialDomainParam: function (record, fieldName, fieldInfo) {
        ...
        var params_all =  ("" + evalContextParam.reports_designer_param_content).split(';');
        var i_split = ''
    	var n = 0
    ...
    _fetchSpecialData: function (record, options) {
    	...
    	if (record && record.model && record.model === "reports.designer.section" && fieldNames.indexOf( 'domain' ) == -1 ) {
