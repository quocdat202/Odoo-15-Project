...
class ReportsDesigner(models.Model):
    _name = "reports.designer"
    _description = "Reports Designer"
    _order = "name,id"
...
See bellow Legends for supported Date and Time Formats.
    ...
    template_name_id = fields.Many2one(comodel_name="ir.attachment", string="Template File Excel", required=False,
                                   ...
                                   domain = "[('res_model', '=', 'reports.designer'), ('res_id', '=', id)]", copy=False
                                   )
    ...
    description = fields.Text('Displayed Description', translate=True, help="This description is displayed in the wizard form when you print the report.")
    description_report = fields.Text('Description', translate=True)
    ...
    reports_designer_param_ids = fields.One2many('reports.designer.param', 'reports_designer_id', string='Report Parameters', copy=False)
    report_id = fields.Integer(compute='_compute_id', string="Report id", store=False)
    reports_designer_param_content = fields.Char(compute='_compute_reports_designer_param_content', string="Report Parameters ", store=False)
    reports_designer_section_ids = fields.One2many('reports.designer.section', 'reports_designer_id', string='Report Sections', copy=True)
    ...
    def _compute_attached_docs_count(self):
        ...
        for doc in self:
            doc.doc_count = Attachment.search_count([
                '&', ('res_model', '=', 'reports.designer'), ('res_id', '=', doc.id)
            ])
    @api.model
    @api.depends('data')
    def _compute_attached_doc_name(self):
        ...
        datafile = Attachment.search_read([
            '&', ('res_model', '=', 'reports.designer'), ('res_id', '=', self.id), ('res_field', '=', 'data')
            ])
        if datafile:
            return datafile[0]['name']
    def _compute_id(self):
        self.report_id = self.id 
    @api.depends('reports_designer_param_ids')
    def _compute_reports_designer_param_content(self):
        params = ''
        if isinstance(self.id, models.NewId):
            self.reports_designer_param_content = params
        else:
            param_ids = self.env['reports.designer.param'].search([('reports_designer_id', '=', self.id)])
            for i in param_ids:
                rel_model = 'false'
    ...
    def create_action(self):
        for report in self:
            context_report1 = "{'model': 'reports.designer', 'id': %d}" % (report.id,)
            context_report2 = "{'id': %d, 'model': 'reports.designer'}" % (report.id,)
            action_id = self.env['ir.actions.act_window'].search(["&",('res_model', '=', 'reports_designer_wizard'),"|",('context', '=', context_report1),('context', '=', context_report2)]).id
            if action_id:
                ...
                action.write(vals)
                 ...
                 view_id = self.env['ir.ui.view'].search([('model', '=', 'reports_designer_wizard')]).id
                 vals = {
                     'name': report.name,
    ...
    def _change_sub_root_model_id(self):
        self.reports_designer_section_ids = [(2, line_id, False) for line_id in self.reports_designer_section_ids.ids]        
        self.email_template_id = False
    def map_params(self, new_report_id):
        reports = self.env['reports.designer.param']
        for param in self.reports_designer_param_ids:
            defaults = {
                'name': param.name,                
                'code': param.code,                
                ...
                'reports_designer_id': new_report_id,                
                }
            reports += param.copy(defaults)
        return self.browse(new_report_id).write({'reports_designer_param_ids': [(6, 0, reports.ids)]})
    def copy(self, default=None):
        self.ensure_one()
        default = dict(default or {}, name=_('%s (copy)') % self.name)
        report = super(ReportsDesigner, self).copy(default)
        if 'reports_designer_param_ids' not in default:
            self.map_params(report.id)
            self.env.cr.execute("""
                    WITH RECURSIVE r AS (
                       SELECT id, parent_id
                       FROM reports_designer_section
                       WHERE reports_designer_id = %s
                       UNION
                       SELECT reports_designer_section.id, reports_designer_section.parent_id
                       FROM reports_designer_section
                          JOIN r
                              ON reports_designer_section.parent_id = r.id
                    )
                    SELECT * FROM r                         
                   """ % (report.id,))
            ...
            if len(cr_ids):
                for i in self.reports_designer_param_ids:
                    for n in report.reports_designer_param_ids:
                        if n.code == i.code:
                             new_id = n.id
                             break
                    query = """
                        UPDATE reports_designer_fields 
                        SET having_param_id = %s 
                        WHERE having_param_id = %s AND reports_designer_section_id IN %s
                    """
    ...
    def create(self, vals):
        if not self.id:
            report_tmp = super(ReportsDesigner, self).create(vals)                     
            check_test, check_text = self.check_report(report_tmp)  
            super(ReportsDesigner, report_tmp).unlink()
            if check_test:
                raise UserError(_(check_text))       
        res = super(ReportsDesigner, self).create(vals)
        if 'reports_designer_param_ids' in vals:
            wizard_model_id = self.env['ir.model'].search([('model','=', 'reports_designer_wizard')],[]).id
            vals_update = {'reports_designer_param_ids':[]} 
            for param_id in res.reports_designer_param_ids:
                if param_id.wizard_param_ir_model_field_id.id == False:
                    seq_number = self.env['ir.sequence'].next_by_code('reports.designer')
                    vals_create = {
                    ...
                    vals_update['reports_designer_param_ids'].append([1, param_id.id, {'wizard_param_ir_model_field_id': record.id}])
            if len(vals_update['reports_designer_param_ids']): 
                super(ReportsDesigner, res).write(vals_update)
        return res
    def write(self, vals):
        wizard_model_id = self.env['ir.model'].search([('model','=', 'reports_designer_wizard')],[]).id
        wizard_data_field_id = self.env['ir.model.fields'].search([('name','=', 'data'), ('model_id','=', wizard_model_id)],[]).id
        vals_update_before = {'reports_designer_param_ids':[]}
        if 'reports_designer_param_ids' in vals:
            unlink_ir_model_fields_ids = [] 
            for param in vals['reports_designer_param_ids']:
                if param[0] in [1,2,3,5]:
                    vals_update_before['reports_designer_param_ids'].append([1, param[1], {'wizard_param_ir_model_field_id': False}])                    
            ...
            res = super(ReportsDesigner, self).write(vals_update_before)
        res = super(ReportsDesigner, self).write(vals)
        if 'reports_designer_param_ids' in vals and vals['reports_designer_param_ids'][0][0] == 6:
            pass
        else:
        ...
        if 'reports_designer_param_ids' in vals:
            vals_update = {'reports_designer_param_ids':[]} 
            for param_id in self.reports_designer_param_ids:
                if param_id.wizard_param_ir_model_field_id.id == False:
                    seq_number = self.env['ir.sequence'].next_by_code('reports.designer')
                    vals_create = {
                    ...
                    vals_update['reports_designer_param_ids'].append([1, param_id.id, {'wizard_param_ir_model_field_id': record.id}])
            if len(vals_update['reports_designer_param_ids']): 
                res = super(ReportsDesigner, self).write(vals_update)
    ...
    def unlink(self):
        ...
        for rec in self:
            context_report1 = "{'model': 'reports.designer', 'id': %d}" % (rec.id,)
            context_report2 = "{'id': %d, 'model': 'reports.designer'}" % (rec.id,)
            action_id = self.env['ir.actions.act_window'].search(["&",('res_model', '=', 'reports_designer_wizard'),"|",('context', '=', context_report1),('context', '=', context_report2)]).id
            if action_id:
                self.env['ir.ui.menu'].search([('action', '=', 'ir.actions.act_window,%d' % (action_id,))]).unlink()
                self.env['ir.actions.act_window'].search(["&",('res_model', '=', 'reports_designer_wizard'),"|",('context', '=', context_report1),('context', '=', context_report2)]).unlink()
        res = super(ReportsDesigner, self).unlink()
        if not len(wizard_param_ir_model_field_ids): 
            records = self.env['ir.model.fields'].search([('name','like', 'x_param_'),('model','=', 'reports_designer_wizard')],[])
            for r in records:
    ...
    def check_report(self, res=None):
        ...
        code_arr = []
        param_dict = {}
        for p in res.reports_designer_param_ids:
        ...
        if len(code_arr) > 1:
            ...
            if len(dup):
                ...
                for code in dup:
                    ...
                    msg_err = msg_err + msg
                result = True
        if len(res.reports_designer_section_ids.ids):
            result_sections, msg_sections_err = self.check_section(res.reports_designer_section_ids, param_dict)
            if result_sections:
    ...
    def check_section(self, res=None, param_dict=None):
        ...
        for section in res:
            ...
            section_field_cell = []
            for field in section.reports_designer_fields_ids:
            ...
            if result_sections:
                ...
                result = True
        return result, msg_err
class ReportsDesignerParam(models.Model):
    _name = "reports.designer.param"
    _description = "Report Excel Params"
    _order = "sequence"
    sequence = fields.Integer(default=10, help="Gives the sequence of this line when displaying.")
    reports_designer_id = fields.Many2one('reports.designer', ondelete='cascade', string='Report excel Reference')
    ...
    def _check_param_ir_model_id(self):
        for rec in self:
            if not rec.param_ir_model_id.id and rec.type_param in ['many2one', 'many2many'] :
                ...
                raise ValidationError(msg)
            return 
class ReportsDesignerSection(models.Model):
    _name = "reports.designer.section"
    ...
    def _compute_report_id(self):
        if self._context.get('report_id'):
            ...
            if self.reports_designer_id:
                self.report_id = self.reports_designer_id.report_id 
            if self.parent_id:
                self.report_id = self.parent_id.report_id 
    @api.model    
    def _default_reports_designer_param_content(self):
        return self._context.get('reports_designer_param_content')
    @api.model    
    def _compute_reports_designer_param_content(self):
        if self._context.get('reports_designer_param_content'):
            self.reports_designer_param_content = self._context.get('reports_designer_param_content')
        else:
            if self.reports_designer_id:
                self.reports_designer_param_content = self.reports_designer_id.reports_designer_param_content 
            if self.parent_id:
                self.reports_designer_param_content = self.parent_id.reports_designer_param_content 
    @api.model    
    ...
    reports_designer_id = fields.Many2one('reports.designer', string='Report excel Reference', ondelete='cascade')
    parent_id = fields.Many2one('reports.designer.section', ondelete='cascade', string='Parent Section')
    children_ids = fields.One2many('reports.designer.section', 'parent_id', string='Children Sections', copy=True)
    report_id = fields.Integer(compute='_compute_report_id', string="Report id", default=_default_compute_report_id, store=False)
    reports_designer_param_content = fields.Char(compute='_compute_reports_designer_param_content', string="Report Parameters", default=_default_reports_designer_param_content, store=False)
    ...
    reports_designer_fields_ids = fields.One2many('reports.designer.fields', 'reports_designer_section_id', string='Report Excel Fields', copy=True)
    ...
    reports_designer_fields_sql_ids = fields.One2many('reports.designer.fields.sql', 'reports_designer_section_id', string='Report Excel Fields SQL', copy=True)
    ...
    def _set_root_model_name(self):
        if self._context.get('section'):
            ...
            self.reports_designer_fields_ids = [(2, line_id, False) for line_id in self.reports_designer_fields_ids.ids]
            self.domain = '[]'    
        else:
            if self.section:
                if self.root_model_id:
    ...
    def _check_start_end(self):
        ...
        for section in self:
            ...
            if row_start > row_end or CheckCell.column_index_from_string(column_start) > CheckCell.column_index_from_string(column_end):
                ...
                raise ValidationError(msg)
        return 
class ReportsDesignerFields(models.Model):
    _name = "reports.designer.fields"
    ...
    def _compute_aggregate_ids(self):
        for field in self:
            ...
            field.aggregate_ids = self.env['reports.designer.aggregate'].search(domain,[]).mapped('id')
            domain = []
            if field._context.get('report_id'):
                report_id = field._context.get('report_id')
            else:
                if field.reports_designer_section_id.report_id:
                    report_id = field.reports_designer_section_id.report_id
                else:
                    report_id = 0
            if field.field_type in ('date',) and field.aggregate_id.code != 'count':
                domain = [('reports_designer_id','=', report_id),('type_param','in', ('date',))]
            elif field.field_type in ('date',) and field.aggregate_id.code == 'count':
                domain = [('reports_designer_id','=', report_id),('type_param','in', ('integer','float',))]
            elif field.field_type in ('datetime',) and field.aggregate_id.code != 'count':
                domain = [('reports_designer_id','=', report_id),('type_param','in', ('datetime',))]
            elif field.field_type in ('datetime',) and field.aggregate_id.code == 'count':
                domain = [('reports_designer_id','=', report_id),('type_param','in', ('integer','float',))]
            else:
                domain = [('reports_designer_id','=', report_id),('type_param','in', ('integer','float',))]
            res_having_param = self.env['reports.designer.param'].search(domain,[])
    ...
    reports_designer_section_id = fields.Many2one('reports.designer.section', string='Section', ondelete='cascade', help="Report excel Section Reference.")
    ...
    aggregate_ids = fields.Many2many('reports.designer.aggregate', string='Aggregates', compute='_compute_aggregate_ids')
    aggregate_id = fields.Many2one('reports.designer.aggregate', string='Aggregate', help="Aggregate Functions.")
    ...
    having_param_ids = fields.Many2many('reports.designer.param', compute='_compute_aggregate_ids')
    having_param_id = fields.Many2one('reports.designer.param', string='Parameter', ondelete='cascade')
...
\u0023   result = cell(H7) ''',
    ...
    def _onchange_having_param_domain(self):
        ...
        if self._context.get('report_id'):
            ...
            if self.reports_designer_section_id.report_id:
                report_id = self.reports_designer_section_id.report_id
            else:
                report_id = 0
        if self.field_type in ('date',) and self.aggregate_id.code != 'count':
            return {'domain': {'having_param_id': [('reports_designer_id','=', report_id),('type_param','in', ('date',))]}}
        if self.field_type in ('date',) and self.aggregate_id.code == 'count':
            return {'domain': {'having_param_id': [('reports_designer_id','=', report_id),('type_param','in', ('integer','float',))]}}
        elif self.field_type in ('datetime',) and self.aggregate_id.code != 'count':
            return {'domain': {'having_param_id': [('reports_designer_id','=', report_id),('type_param','in', ('datetime',))]}}
        elif self.field_type in ('datetime',) and self.aggregate_id.code == 'count':
            return {'domain': {'having_param_id': [('reports_designer_id','=', report_id),('type_param','in', ('integer','float',))]}}
        else:
            return {'domain': {'having_param_id': [('reports_designer_id','=', report_id),('type_param','in', ('integer','float'))]}}
...
class ReportsDesignerFieldsSQL(models.Model):
    _name = "reports.designer.fields.sql"
    ...
    reports_designer_section_id = fields.Many2one('reports.designer.section', string='Section', ondelete='cascade', help="Report excel Section Reference.")
    root_model_id = fields.Integer(string='Report Root Model', default=_compute_root_model)
class ReportsDesignerAggregate(models.Model):
    _name = "reports.designer.aggregate"
    _description = " "
    code = fields.Char(string='Code')
    name = fields.Char(string='Aggregate Name')
class ReportsDesignerWizardReportMenu(models.TransientModel):
    _name = 'reports.designer.wizard.report.menu.create'
    _description = " "
    menu_id = fields.Many2one('ir.ui.menu', string='Parent Menu', required=True, ondelete='cascade')
    name = fields.Char(string='Menu Name', required=True)
    def menu_create(self):
        for menu in self:
            context_report1 = "{'model': 'reports.designer', 'id': %d}" % (self.env.context.get('active_id'),)
            context_report2 = "{'id': %d, 'model': 'reports.designer'}" % (self.env.context.get('active_id'),)
            action_id = self.env['ir.actions.act_window'].search(["&",('res_model', '=', 'reports_designer_wizard'),"|",('context', '=', context_report1),('context', '=', context_report2)]).id
            if not action_id:
                reports_designer_name = self.env['reports.designer'].search([('id', '=', self.env.context.get('active_id'))]).name
                view_id = self.env['ir.ui.view'].search([('model', '=', 'reports_designer_wizard')])
                vals = {
                    'name': reports_designer_name,
                    'res_model': 'reports_designer_wizard',
                    'view_mode': 'form',
                    'target': 'new',
                    'view_id': view_id.id,
