...
class ApiRequest(JsonRequest, WebRequest):
    ...
    * a ``str`` or ``unicode``, will be wrapped in a Response object and
    ...
    def _json_response(self, result=None, error=None):
        # Must be added, because by default Odoo does not have an error parameter in the JSON Endpoint
        # ---------------------Begin of New Part----------------------------#
        status = 200
        ...
        # Must be added, because by default odoo does not have a STATUS CODE on the JSON Endpoint
        # ---------------------Begin of New Part----------------------------#
        return Response(
            response=body,
            status=status,
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(
                exception, (odoo.exceptions.Warning, SessionExpiredException)
            ):
...
def get_request(self, httprequest):
    if (Api_Prefix in httprequest.path) and (
        httprequest.mimetype == "application/json"
    ):
